        ...
        <table class="table">
            ...
            <tfoot>
                ...
                @php
                    ...
                    $wallet_balance = Auth::check() ? Auth::user()->balance : 0;
                    $remaining_price = max(0, $total - $wallet_balance);
                @endphp

                    @if (Auth::check() && get_setting('wallet_system') == 1)
                    <tr id="wallet-balance-row" style="display: none;">
                        <th>{{ translate('Wallet Balance') }}</th>
                        <td class="text-right">
                            <span class="font-italic">{{ single_price($wallet_balance) }}</span>
                        </td>
                    </tr>
                    <tr id="remaining-price-row" style="display: none;">
                        <th>{{ translate('Total') }}</th>
                        <td class="text-right">
                            <span class="font-italic">{{ single_price($remaining_price) }}</span>
                        </td>
                    </tr>
                    @endif

                    <tr class="cart-total" id="total-row">
