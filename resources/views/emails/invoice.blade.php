				...
				@if($shipping_address)
					...
					<tr><td><address>
						<strong class="text-main">
							{{ translate('Name') }}:
						</strong> {{ $shipping_address->name ?? "" }} <br>
						<strong class="text-main">
							{{ translate('Email') }}:
						</strong> {{ $shipping_address->email ?? "" }}<br>
						<strong class="text-main">
							{{ translate('Phone') }}:
						</strong> {{ $shipping_address->phone ?? "" }}, <strong class="text-main">
							{{ translate('Type') }}:
						</strong> {{ $shipping_address->address_type ?? '' }} <br>
						<strong class="text-main">
							{{ translate('State') }}:
						</strong> {{ $shipping_address->state ?? "" }}, <strong class="text-main">
							{{ translate('City') }}:
						</strong> {{ $shipping_address->city ?? "" }}, <strong class="text-main">
							{{ translate('Block') }}:
						</strong> {{ $shipping_address->bloc ?? "" }}, <strong class="text-main">
							{{ translate('Avenue') }}:
						</strong> {{ $shipping_address->avenue ?? "" }}, <strong class="text-main">
							{{ translate('Street') }}:
						</strong> {{ $shipping_address->street ?? "" }} <br>
						<strong class="text-main">
							{{ translate('House') }}:
						</strong> {{ $shipping_address->house ?? "" }}, <strong class="text-main">
							{{ translate('Address') }}:
						</strong> {{ $shipping_address->address ?? "" }}, <strong class="text-main">
							{{ translate('Address Label') }}:
						</strong> {{ $shipping_address->address_label ?? "" }}, 
						
							{{ translate('Building Number') }}:
						@if($shipping_address->address_type == "apartment" || $shipping_address->address_type == "office")
								<strong class="text-main">
									{{ translate('building Number. or building name') }}:
								</strong> 
						@else
								<strong class="text-main">
					...
					@endif
						{{ $shipping_address->building_number ?? "" }} <br>
						@if(json_decode($order->shipping_address)->address_type != "house")
								<strong class="text-main">
									{{ translate('Floor') }}:
								</strong> {{ $shipping_address->floor ?? '' }}
								<strong class="text-main">
									{{ translate('Apt.Number') }}:
								</strong> {{ $shipping_address->floor ?? '' }}
								@endif

						{{-- {{ $shipping_address->address }}, {{ $shipping_address->city }}, @if(isset($shipping_address->state)) {{ $shipping_address->state }}@endif<br> --}}
