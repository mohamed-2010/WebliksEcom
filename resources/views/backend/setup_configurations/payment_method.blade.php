...
{{--
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header ">
                    <h5 class="mb-0 h6">{{translate('BookeeyPay Credential')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('payment_method.update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="payment_method" value="bookeeypay">
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="BOOKEEYPAY_MID">
                            <div class="col-md-4">
                                <label class="col-from-label">{{translate('BookeeyPay MID')}}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="BOOKEEYPAY_MID" value="{{  env('BOOKEEYPAY_MID') }}" placeholder="{{translate('BookeeyPay MID')}}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="BOOKEEYPAY_SECRET_KEY">
                            <div class="col-md-4">
                                <label class="col-from-label">{{translate('BookeeyPay Secret key')}}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="BOOKEEYPAY_SECRET_KEY" value="{{  env('BOOKEEYPAY_SECRET_KEY') }}" placeholder="{{translate('BookeeyPay Secret key')}}" required>
                            </div>
                        </div>
                        <div class="form-group mb-0 text-right">
                            <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header ">
                    <h5 class="mb-0 h6">{{translate('UPayment Credential')}}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('payment_method.update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="payment_method" value="upayment">
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="UPAYMENT_MID">
                            <div class="col-md-4">
                                <label class="col-from-label">{{translate('UPayment Key')}}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="UPAYMENT_MID" value="{{  env('UPAYMENT_MID') }}" placeholder="{{translate('UPayment MID')}}" required>
                            </div>
                            <div class="col-12 d-flex justify-content-between m-2">
                                <!-- Switch for enable kenet type -->
                                <div class="col-md-4 d-flex justify-content-center align-items-center">
                                    <label class="col-from-label m-2">{{translate('Enable Kenet')}}</label>
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input type="hidden" name="types[]" value="upayment_kenet">
                                        <input value="1" name="upayment_kenet" type="checkbox" @if (get_setting('upayment_kenet') == 1)
                                            checked
                                        @endif>
                                        <span class="slider round"></span>
                                    </label>
                                </div>
                                <!-- Switch for enable visa type -->
                                <div class="col-md-4 d-flex justify-content-center align-items-center">
                                    <label class="col-from-label m-2">{{translate('Enable Visa')}}</label>
                                    <label class="aiz-switch aiz-switch-success mb-0">
                                        <input type="hidden" name="types[]" value="upayment_visa">
                                        <input value="1" name="upayment_visa" type="checkbox" @if (get_setting('upayment_visa') == 1)
                                            checked
                                        @endif>
                                        <span class="slider round"></span>
                                    </label>
...
{{-- <div class="col-md-6">
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Network UAE Gateway Credential') }}</h5>
        </div>
        <div class="card-body">
            <form class="form-horizontal" action="{{ route('payment_method.update') }}" method="POST">
                @csrf
                <input type="hidden" name="payment_method" value="uae_gateway">

                <div class="form-group row">
                    <input type="hidden" name="types[]" value="UAE_GATEWAY_API_URL">
                    <div class="col-md-4">
                        <label class="col-from-label">{{ translate('API URL') }}</label>
                    </div>
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="UAE_GATEWAY_API_URL" value="{{ env('UAE_GATEWAY_API_URL') }}" placeholder="{{ translate('e.g. https://uae-gateway.test/api/checkout') }}" required>
                    </div>
                </div>

                <div class="form-group row">
                    <input type="hidden" name="types[]" value="UAE_GATEWAY_API_KEY">
                    <div class="col-md-4">
                        <label class="col-from-label">{{ translate('API Key') }}</label>
                    </div>
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="UAE_GATEWAY_API_KEY" value="{{ env('UAE_GATEWAY_API_KEY') }}" placeholder="{{ translate('Enter your API Key') }}" required>
                    </div>
                </div>

                <div class="form-group row">
                    <input type="hidden" name="types[]" value="UAE_GATEWAY_API_SECRET">
                    <div class="col-md-4">
                        <label class="col-from-label">{{ translate('API Secret') }}</label>
                    </div>
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="UAE_GATEWAY_API_SECRET" value="{{ env('UAE_GATEWAY_API_SECRET') }}" placeholder="{{ translate('Enter your API Secret') }}" required>
                    </div>
                </div>

                <div class="form-group row">
                    <input type="hidden" name="types[]" value="UAE_GATEWAY_WEBHOOK_SECRET">
                    <div class="col-md-4">
                        <label class="col-from-label">{{ translate('API Webhook Secret') }}</label>
                    </div>
                    <div class="col-md-8">
                        <input type="text" class="form-control" name="UAE_GATEWAY_WEBHOOK_SECRET" value="{{ env('UAE_GATEWAY_WEBHOOK_SECRET') }}" placeholder="{{ translate('Enter your API Webhook Secret') }}" required>
                    </div>
                </div>
...
<!-- End UAE Gateway -->
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0 h6">{{ translate('Tabby Gateway Credential') }}</h5>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('payment_method.update') }}" method="POST">
                        @csrf
                        <input type="hidden" name="payment_method" value="tabby_gateway">

                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="TABBY_PUBLIC_KEY">
                            <div class="col-md-4">
                                <label class="col-from-label">{{ translate('Public Key') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="TABBY_PUBLIC_KEY" value="{{ env('TABBY_PUBLIC_KEY') }}" placeholder="{{ translate('Enter your Public Key') }}" required>
                            </div>
                        </div>

                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="TABBY_SECRET_KEY">
                            <div class="col-md-4">
                                <label class="col-from-label">{{ translate('Secret Key') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="TABBY_SECRET_KEY" value="{{ env('TABBY_SECRET_KEY') }}" placeholder="{{ translate('Enter your Secret Key') }}" required>
                            </div>
                        </div>

                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="TABBY_MERCHANT_CODE">
                            <div class="col-md-4">
                                <label class="col-from-label">{{ translate('Merchant Code') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="text" class="form-control" name="TABBY_MERCHANT_CODE" value="{{ env('TABBY_MERCHANT_CODE') }}" placeholder="{{ translate('Enter your Merchant Code') }}" required>
                            </div>
                        </div>
                        <!-- Add minimum amount field -->
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="TABBY_MIN_AMOUNT">
                            <div class="col-md-4">
                                <label class="col-from-label">{{ translate('Minimum Amount') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="number" class="form-control" name="TABBY_MIN_AMOUNT" value="{{ get_setting('TABBY_MIN_AMOUNT', 200) }}" placeholder="{{ translate('Minimum amount for Tabby payments') }}" required>
                            </div>
                        </div>

                        <!-- Add maximum amount field -->
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="TABBY_MAX_AMOUNT">
                            <div class="col-md-4">
                                <label class="col-from-label">{{ translate('Maximum Amount') }}</label>
                            </div>
                            <div class="col-md-8">
                                <input type="number" class="form-control" name="TABBY_MAX_AMOUNT" value="{{ get_setting('TABBY_MAX_AMOUNT', 9000) }}" placeholder="{{ translate('Maximum amount for Tabby payments') }}" required>
                            </div>
                        </div>
