        ...
        <div class="card-header row gutters-5">
            ...
            <div class="col-auto">
                <div class="form-group mb-0">
                    ...
                    <a href="{{ route('salesReport.index') }}" class="btn btn-secondary">{{ translate('Reset') }} </a>
                </div>
            </div>
            <!-- export excel button -->
            <div class="col-auto">
                <div class="form-group mb-0">
                    <a id="export_sales" class="badge-success btn" onclick="bulkExport()">{{ translate('Export Excel') }}</a>
                </div>
            </div>
        </div>
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="md">{{ translate('Branch') }}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @php
                                $city_id = json_decode($order->shipping_address)->city_id ?? null;
                            @endphp
                            {{
                                $city_id != null ?
                                \App\Models\City::find($city_id)->branch_cities->pranche->name ?? ""
                                : ""
                            }}
                        </td>
                        <td>
                            @if ($order->user != null)
                                {{ $order->user->name }}
                            @else
                                Guest ({{ count(\App\Models\Guest::where('id',$order->guest_id)->get()) > 0 ?  \App\Models\Guest::where('id',$order->guest_id)->get()[0]->name : '' }})
                            @endif
                        </td>
                        <td>
                            @if($order->shop)
...
//        }
        ...
        function bulkExport() {
            ...
            window.location.href = "{{route('export-sales-report')}}" + "?" + queryString;
        }
    </script>
@endsection
