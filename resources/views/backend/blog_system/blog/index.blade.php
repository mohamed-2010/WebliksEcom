            ...
            <table class="table mb-0 aiz-table">
                <thead>
                    <tr>
                        ...
                        <th>{{ translate('URL') }} ({{ strtoupper($language->code) }})</th>
                        @endforeach
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @foreach (\App\Models\Language::all() as $key => $language)
                        <td>
                            <a href="{{ url($language->code . '/blog/' . $blog->getTranslation('url', $language->code)) }}" target="_blank">
                                {{ url($language->code . '/blog/' . $blog->getTranslation('url', $language->code)) }}
                            </a>
                            <button class="btn btn-sm btn-primary copy-btn" data-url="{{ url($language->code . '/blog/' . $blog->getTranslation('url', $language->code)) }}" title="Copy URL">
                        ...
                        <td class="text-right">
                            @can('edit_blog')
                                <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{ route('blog.edit', ['blog'=>$blog->id, 'lang'=> env('DEFAULT_LANGUAGE')])}}" title="{{ translate('Edit') }}">
    ...
    <script type="text/javascript">
        ...
        document.body.addEventListener('click', function(event) {
            if (event.target.closest('.copy-btn')) {
                let button = event.target.closest('.copy-btn');
                let url = button.getAttribute('data-url');

                navigator.clipboard.writeText(url).then(() => {
                    AIZ.plugins.notify('success', 'Copied to clipboard!');
                }).catch(err => {
                    AIZ.plugins.notify('danger', 'Failed to copy URL.');
