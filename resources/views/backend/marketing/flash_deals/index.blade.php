            ...
            <thead>
                <tr>
                    ...
                    @foreach (\App\Models\Language::all() as $language)
                        <th data-breakpoints="lg">{{ translate('Page Link') }} ({{ $language->name }})</th>
                    @endforeach
            ...
            <tbody>
                @foreach($flash_deals as $key => $flash_deal)
                    <tr>
                        ...
                        @foreach (\App\Models\Language::all() as $language)
                            <td>
                                <a href="{{ url($language->code . '/flash-deal/' . $flash_deal->slug) }}" target="_blank">
                                    {{ url($language->code . '/flash-deal/' . $flash_deal->slug) }}
                                </a>
                                <button class="btn btn-sm btn-outline-secondary ml-2" onclick="copyToClipboard('{{ url($language->code . '/flash-deal/' . $flash_deal->slug) }}')" title="Copy Link">
