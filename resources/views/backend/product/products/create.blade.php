...
<div class="">
    ...
    @endif
    @if(count(App\Models\Product::all()) >= get_setting('products_count'))
        <div class="alert alert-danger">
            {{ translate('Your product limit has been reached') }}
        </div>
    @else
    <form class="form form-horizontal mar-top" {{--  action="{{route('products.store')}}" method="POST" --}} enctype="multipart/form-data" id="choice_form">
        <div class="row gutters-5">
            <div class="col-lg-8">
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                ...
                                <small class="text-muted">{{translate('This image is visible in all product box. Use 600x600 sizes image. Keep some blank space around main object of your image as we had to crop some edge in different devices to make it responsive.')}}</small>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0 h6">{{translate('Category AddOns')}}</h5>
                    </div>
                    <div class="card-body">
                        <div class="form-group row gutters-5">
                            <select class="form-control aiz-selectpicker" data-live-search="true" data-selected-text-format="count" name="category_add_on_ids[]" id="category_add_on_ids" multiple>
                                @foreach (\App\Models\CategoryAddOn::orderBy('name', 'asc')->get() as $key => $category_addon)
                                <option value="{{ $category_addon->id }}">{{ $category_addon->name }}</option>
                                {{-- <option  value="{{ $category_addon->id }}" data-content="<span><span class='size-15px d-inline-block mr-2 rounded border'></span><span>{{ $category_addon->name }}</span></span>"></option> --}}
                                @endforeach
                            </select>
                            {{-- <div class="col-md-8">
                                <select name="choice_attributes[]" id="choice_attributes" class="form-control aiz-selectpicker" data-selected-text-format="count" data-live-search="true" multiple data-placeholder="{{ translate('Choose Attributes') }}">
                        ...
                        <div class="customer_choice_options" id="category_addons">
            ...
            <div class="col-12">
                <div class="btn-toolbar float-right mb-3" role="toolbar" aria-label="Toolbar with button groups">
                    <div class="btn-group mr-2" role="group" aria-label="Third group">
                        <button type="submit" name="button" value="unpublish" class="btn btn-primary action-btn" id="unpublish">{{ translate('Save & Unpublish') }}</button>
                    </div>
                    <div class="btn-group" role="group" aria-label="Second group">
                        <button type="submit" name="button" value="publish" class="btn btn-success action-btn" id="publish">{{ translate('Save & Publish') }}</button>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $(document).on("submit", "#choice_form", function(e){
        ...
        $.map(unindexed_array, function(n, i){
                indexed_array.push({name:n['name'], value:n['value']});
        });

        indexed_array.push({name:'button', value:status});

        $.ajax({
            url: '{{ route('products.store_product') }}',
            method: "POST",
            data: $.param(indexed_array),
            beforeSend: function () {
