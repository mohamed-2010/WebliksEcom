        ...
        @if($type != 'Seller' && auth()->user()->can('add_new_product'))
            @if(count(App\Models\Product::all()) >= get_setting('products_count'))
                <div class="col text-right">
                    <a href="#" class="btn btn-circle btn-danger disabled" title="{{translate('You have reached your product upload limit')}}">
                        <h5 style="font-size: 1rem;">{{count(App\Models\Product::all())}}/{{get_setting('products_count')}}</h5>
                        <span> {{translate('The Limit Has Been')}}</span>
                    </a>
                </div>
            @else
                <div class="col text-right">
                        ...
                        <td>
                            @php
                                ...
                                if($product->variant_product) {
                                    ...
                                    if($count > 3) {
                                        echo '<span class="text-muted">+ '.($count-3).' more variants</span>';
                                    }
                                }
                                else {
