						...
						@foreach($attributes as $key => $attribute)
							<tr>
								...
								<td>
									...
									<span class="badge badge-inline badge-md bg-soft-dark">{{ $value->get_translation('value')  }}</span>
									@endforeach
								</td>
								<td class="text-right">
									@can('view_product_attribute_values')
										<a class="btn btn-soft-info btn-icon btn-circle btn-sm" href="{{route('attributes.show',$attribute->id, ['lang'=>env('DEFAULT_LANGUAGE')])}}" title="{{ translate('Attribute values') }}">
				...
				<div class="card-body">
					<form action="{{ route('attributes.store') }}" method="POST">
						...
						<div class="form-group mb-3">
							<label for="name">{{translate('Name')}} ({{ $language->name }})</label>
