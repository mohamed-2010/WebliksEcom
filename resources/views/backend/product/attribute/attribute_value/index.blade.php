                ...
                <table class="table aiz-table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{$key+1}}</td>
                            <td>
                                {{ $attribute_value->get_translation('value') }}
                            </td>

                            <td class="text-right">
                                @can('edit_product_attribute_value')
                                    <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('edit-attribute-value', ['id'=>$attribute_value->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" title="{{ translate('Edit') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route('store-attribute-value') }}" method="POST">
                        ...
                        <div class="form-group mb-3">
                            <label for="name">{{translate('Attribute Value')}} ({{ $language->name }})</label>
