...
@php
    ...
    if (is_array($details) && count($details) > 0 && isset($details[0]['min_buy'])) {
        $min_buy = $details[0]['min_buy'];
    }
@endphp
...
<div class="form-group row">
    ...
    <div class="col-lg-9">
       <input type="number" lang="en" min="0" step="0.01" name="min_buy" class="form-control" value="{{ $min_buy }}" required>
    </div>
</div>
<div class="product-choose-list">
    <div class="product-choose">
        <div class="form-group row">
            ...
            <div class="col-lg-9">
                <select name="category_ids[]" class="form-control product_id aiz-selectpicker" data-live-search="true" data-selected-text-format="count" required multiple id="category_ids">
                    @foreach($categories as $key => $category)
                        <option value="{{ $category->id }}"
                            @foreach (json_decode($coupon->details) as $details)
                                @if (isset($details->category_id) && $details->category_id == $category->id)
                                    selected
                                @endif
                            @endforeach
                        >
    ...
    $(document).ready(function() {
        ...
        $('#category_ids').on('change', function() {
            ...
            $.each(categories, function(key, category) {
                if (category.parent_id != 0 && selectedCategories.includes(category.parent_id.toString())) {
                    subCategories.push(category.id);
                }
            });

            $('#category_ids').val([...new Set([...selectedCategories, ...subCategories])]);
