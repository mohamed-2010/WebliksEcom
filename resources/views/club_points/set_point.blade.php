...
<div class="row">
    ...
    <div class="col-lg-4">
          ...
          <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">{{translate('Set Point for products in categories or brands')}}</h5>
            </div>
            <div class="card-body">
                <form class="form-horizontal" action="{{ route('admin.club_point.set_points_for_selected_categories_brands') }}" method="POST">
                    @csrf
                    <div class="form-group row">
                        <div class="col-lg-4">
                            <label class="col-from-label">{{translate('Select Type')}}</label>
                        </div>
                        <div class="col-lg-6">
                            <select class="form-control aiz-selectpicker" name="selected_type" id="selected_type" onchange="selected_form()">
...
@section('script')
<script>
    function selected_form(){
        var selected_type = $('#selected_type').val();
        $.post('{{ route('admin.club_point.get_filter_form') }}',{_token:'{{ csrf_token() }}', selected_type:selected_type}, function(data){
