...
use App\Http\Controllers\Api\V2\ProductBindOracleController;
use Illuminate\Support\Facades\Route;

Route::post('/v2/auth2/verify', [AuthController::class, 'verifyUser']);
Route::post('/images/resize', [AizUploadController::class, 'resizeExistingImages'])->name('images.resize');
...
Route::group(['prefix' => 'v2', 'middleware' => ['app_language']], function() {
    ...
    // This Route For Bind With Oracle System
    Route::group(['middleware' => ['apiAccess']], function () {
        // Retrieve all products
        Route::get('/products', [ProductBindOracleController::class, 'products'])
            ->name('products.index');

        // Update product quantity
        Route::patch('/products/{product}/quantity', [ProductBindOracleController::class, 'updateProductQuantity'])
            ->name('products.updateQuantity');

        // Update product price
        Route::patch('/products/{product}/price', [ProductBindOracleController::class, 'updateProductPrice'])
            ->name('products.updatePrice');

        // Update stock variant price
        Route::patch('/stocks/{productStock}/price', [ProductBindOracleController::class, 'updateStockVariantPrice'])
            ->name('stocks.updatePrice');

        // Update stock variant quantity
        Route::patch('/stocks/{productStock}/quantity', [ProductBindOracleController::class, 'updateStockVariantQuantity'])
            ->name('stocks.updateQuantity');
    });
});
