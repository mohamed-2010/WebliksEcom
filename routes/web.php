...
use App\Http\Controllers\Payment\BookeeypayController;
...
use App\Http\Controllers\Payment\RechargeBookeeypayController;
use App\Http\Controllers\Payment\RechargeupayController;
use App\Http\Controllers\Payment\TabbyPaymentController;
use App\Http\Controllers\Payment\UaePaymentController;
use App\Http\Controllers\Payment\UaePaymentWebhookController;
use App\Http\Controllers\Payment\WalletupayController;
...
use App\Http\Controllers\GoogleMerchantApiController;
...
Route::get('/google-merchant-feed', [AdminProductController::class, 'get_products_feed'])->name('google-merchant-feed');

Route::get('/get-products-facebook-data-feed', [AdminProductController::class, 'get_products_for_facebook_feed'])->name('get_products_for_facebook_feed');

Route::get('/sitemap.xml', function () {
    return response()->file(public_path('sitema.xml'), [
        'Content-Type' => 'application/xml'
    ]);
});

Route::group(['prefix' => LaravelLocalization::setLocale()], function () {
    Route::post('armada-web-hooks', [ShippmentController::class, 'armada_web_hooks']);
    Route::get('/sitemap-generate', [BusinessSettingsController::class, 'sitemap_generator'])->name('sitemap_generator');
    ...
    Route::controller(AizUploadController::class)->group(function () {
        ...
        Route::get('/aiz-uploader/get_uploaded_files', 'get_uploaded_files')->name('aiz-uploader.get_uploaded_files');
    ...
    Route::controller(CheckoutController::class)->group(function () {
        Route::get('/guest', 'guest')->name('checkout.guest');
        Route::any('/delivery_info_guest', 'store_shipping_info_guest')->name('checkout.store_shipping_infostore_guest');
        Route::post('/payment_select_guest', 'store_delivery_info_guest')->name('checkout.store_delivery_info_guest');
        Route::post('/payment_guest', 'checkout_guest')->name('payment.checkout_guest');
        Route::get('/order-confirmed-guest', 'order_confirmed_guest')->name('order_confirmed_guest');
        Route::post('/apply_coupon_code_guest', 'apply_coupon_code_guest')->name('checkout.apply_coupon_code_guest');
    });
    ...
    Route::group(['prefix' => 'checkout'], function() {
        Route::controller(CheckoutController::class)->group(function () {
            ...
            Route::post('/remove_coupon_code_guest', 'remove_coupon_code')->name('checkout.remove_coupon_code_guest');
    ...
    Route::group(['middleware' => ['customer', 'verified', 'unbanned']], function() {
        ...
        Route::group(['prefix' => 'checkout'], function() {
            Route::controller(CheckoutController::class)->group(function () {
                ...
                Route::get('/get_wallet_and_total', 'getWalletAndTotal')->name('checkout.get_wallet_and_total');
                //Club point
    ...
    //bookeeypay
    Route::controller(BookeeypayController::class)->group(function () {
        Route::get('/bookeeypay/success','success')->name('bookeeypay.success');
        Route::get('/bookeeypay/fail','fail')->name('bookeeypay.fail');
    });

    Route::controller(RechargeBookeeypayController::class)->group(function () {
        Route::get('recharge/bookeeypay/success','success')->name('rechargebookeeypay.success');
        Route::get('recharge/bookeeypay/fail','fail')->name('rechargebookeeypay.fail');
    });

    //uae payment
    // Route::controller(UaePaymentController::class)->group(function () {
    //     Route::any('/uae/success','success')->name('uae.payment.success')->withoutMiddleware([VerifyCsrfToken::class]);
    //     Route::any('/uae/fail','fail')->name('uae.payment.fail')->withoutMiddleware([VerifyCsrfToken::class]);
    // });
    // Route::post('/networkUae/webhook/payment', [UaePaymentWebhookController::class, 'handleWebhook']);


    //tabby payment
    Route::controller(TabbyPaymentController::class)->group(function () {
        Route::any('/tabby/success','success')->name('tabby.payment.success')->withoutMiddleware([VerifyCsrfToken::class]);
        Route::any('/tabby/fail','fail')->name('tabby.payment.fail')->withoutMiddleware([VerifyCsrfToken::class]);
        Route::any('/tabby/callBack','callBack')->name('tabby.payment.callBack')->withoutMiddleware([VerifyCsrfToken::class]);

    });
    Route::post('/tabby/webhook', [TabbyPaymentController::class, 'webhook'])
    ->name('tabby.webhook')->withoutMiddleware([VerifyCsrfToken::class]);




    //upayment
    Route::controller(UpayController::class)->group(function () {
        Route::any('/upay/success','success')->name('upay.success')->withoutMiddleware([VerifyCsrfToken::class]);
        Route::any('/upay/fail','fail')->name('upay.fail')->withoutMiddleware([VerifyCsrfToken::class]);
    });
    Route::controller(WalletupayController::class)->group(function () {
        Route::get('/walletupay/success','success')->name('walletupay.success');
        Route::get('/walletupay/fail','fail')->name('walletupay.fail');
    });

    Route::controller(RechargeupayController::class)->group(function () {
        Route::get('recharge/upay/success','success')->name('rechargeupay.success');
        Route::get('recharge/upay/fail','fail')->name('rechargeupay.fail');
    });
