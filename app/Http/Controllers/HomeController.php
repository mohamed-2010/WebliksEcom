...
class HomeController extends Controller
{
    ...
    private function generateLabelFromLink($link)
    {
        ...
        return ucfirst($lastSegment) ?: 'Default Label';
    }
    
    public function migrateHeaderMenuLinks()
    {
        // Fetch header menu links from business_settings
        $headerLinks = DB::table('business_settings')
            ->where('type', 'header_menu_links')
            ->value('value');
        if ($headerLinks) {
            $linksArray = json_decode($headerLinks, true);
            if (is_array($linksArray)) {
                // Check for existing entries in home_navbar_translations
                $existingLinks = DB::table('home_navbar_translations')->pluck('link')->toArray();
                $navbarTranslations = [];
                foreach ($linksArray as $link) {
                     if (!in_array($link, $existingLinks)) {
                        $navbarTranslations[] = [
                            'label' => $this->generateLabelFromLink($link),
                            'link' => $link,
                            'lang' => 'en',
                        ];
                    }
                }
                if (!empty($navbarTranslations)) {
                    // Insert new translations
                    DB::table('home_navbar_translations')->insert($navbarTranslations);
                    // Optional: Delete old entry
                    DB::table('business_settings')
                        ->where('type', 'header_menu_links')
                        ->delete();

                } else {
                    //  "No new links to migrate.";
                }
            } else {
                //  "Failed to decode JSON.";
            }
        } else {
            //  "No header menu links found in the business_settings table.";
        }
    }
    ...
    public function trackOrder(Request $request)
    {
        if ($request->has('order_code')) {
            ...
            if ($order != null) {
                if($order->shipping_company == "mashkor") {
                    $shippment = new ShippmentController;
                    $shippment_data = $shippment->getMashkorOrderDetails($order->traking_order_id, $order->branch_id);
                    return view('frontend.track_order', compact('order', 'shippment_data'));
                }else if($order->shipping_company == "armada") {
                    $shippment = new ShippmentController;
                    $shippment_data = $shippment->getArmadaOrderDetails($order->traking_order_id);
                    return view('frontend.track_order', compact('order', 'shippment_data'));
                }else if($order->shipping_company == "quick") {
                    $shippment = new ShippmentController;
                    $shippment_data = $shippment->getQuickOrderDetails($order->traking_order_id);
                    return view('frontend.track_order', compact('order', 'shippment_data'));
                }
    ...
    public function variant_price(Request $request)
    {
        ...
        //check if the addons quantity is more than 0
        foreach($request->all() as $key => $value) {
            if(strpos($key, 'product_addon_id_') !== false) {
                //replace product_addon_id_ with empty string and add it to the array as id and quantity
                // $addons[] = [
                //     'id' => str_replace('product_addon_id_', '', $key),
                //     'quantity' => $value,
                //     'price' => CategoryAddon::where('id', str_replace('product_addon_id_', '', $key))->first()->price
                // ];
                $add_on_price += CategoryAddon::where('id', str_replace('product_addon_id_', '', $key))->first()->price * $value;
            }
        }
        ...
        return array(
            'price' => single_price(($price * $request->quantity) + $add_on_price),
            'quantity' => $quantity,
