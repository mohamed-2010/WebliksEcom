...
class CartController extends Controller
{
    public function summary(Request $request)
    {
        ...
        $items = auth()->user() != null ? auth()->user()->carts : Cart::where('temp_user_id', $request->user_id)->get();
        if ($items->isEmpty()) {
    ...
    {
        $items = auth()->user() != null ? auth()->user()->carts : Cart::where('temp_user_id', $request->user_id)->count();

        return response()->json([
            'count' => auth()->user() != null ? count($items) : $items,
            'status' => true,
        ]);
    }
    ...
    {
        $owner_ids = auth()->user() != null ? Cart::where('user_id', auth()->user()->id)->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray()
            : Cart::where('temp_user_id', $request->user_id)->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray();
        $currency_symbol = currency_symbol();
        $shops = [];
        if (!empty($owner_ids)) {
            foreach ($owner_ids as $owner_id) {
                $shop = array();
                $shop_items_raw_data = auth()->user() != null ? Cart::where('user_id', auth()->user()->id)->where('owner_id', $owner_id)->get()->toArray()
                    : Cart::where('temp_user_id', $request->user_id)->where('owner_id', $owner_id)->get()->toArray();
    ...
    public function add(Request $request)
    {
        ...
        Cart::updateOrCreate([
            ...
            'temp_user_id' => auth()->user() == null ? $request->user_id : null,
