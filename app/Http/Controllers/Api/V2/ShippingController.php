...
class ShippingController extends Controller
{
    ...
    public function shipping_cost(Request $request)
    {
        if(auth()->user() != null) {
            ...
            $carts = Cart::where('temp_user_id', $request->user_id)
                ->get();
        }
        ...
        foreach ($request->seller_list as $key => $seller) {
            ...
            if(auth()->user() != null) {
                ...
                $carts = Cart::where('temp_user_id', $request->user_id)->where("owner_id", $seller['seller_id'])
                ->get();
            }
        ...
        if(auth()->user() != null) {
            ...
            $total_shipping_cost = Cart::where('temp_user_id', $request->user_id)->sum('shipping_cost');
        }
    ...
    public function getDeliveryInfo(Request $request)
    {
        if(auth()->user() != null) {
            ...
            $owner_ids = Cart::where('temp_user_id', $request->user_id)->select('owner_id')->groupBy('owner_id')->pluck('owner_id')->toArray();
        }
        ...
        if (!empty($owner_ids)) {
            foreach ($owner_ids as $owner_id) {
                ...
                if(auth()->user() != null) {
                    ...
                    $shop_items_raw_data = Cart::where('temp_user_id', $request->user_id)->where('owner_id', $owner_id)->get()->toArray();
                }
...
*/
                ...
                $shop['carriers'] = seller_base_carrier_list($owner_id, $request->user_id);
                $shop['pickup_points']=[];
