...
function convert_currency_prive_with_api($base, $symbol, $price) {
    $url = "https://api.exchangerate.host/latest?base=".$base."&symbols=".$symbol."&amount=".$price;
    $ch = curl_init();
...
//convert currency rate with api and change it in database with cron job
if (!function_exists('convert_currency_rate_with_api')) {
    function convert_currency_rate_with_api()
    {
        $currencies = Currency::where('code', '!=', 'USD')->get();
        foreach ($currencies as $currency) {
            $currency->exchange_rate = convert_currency_prive_with_api('USD', $currency->code, 1);
            $currency->save();
        }
    }
}
...
if (!function_exists('calculate_coupon_discount')) {
    /**
     * Calculate the coupon discount based on the provided parameters.
     *
     * @param string $code The coupon code.
     * @param int|null $user_id The ID of the authenticated user, null for guests.
     * @param int|null $guest_id The ID of the guest user, null for authenticated users.
     * @param string $temp_user_id The temporary user ID for guests.
     * @return array An array containing 'discount' and 'carts'.
     */
    function calculate_coupon_discount($code, $user_id = null, $guest_id = null, $temp_user_id = null, $useForCalculation = false)
    {
        $coupon = Coupon::where('code', $code)->first();
        $coupon_discount = 0;

        if (!$coupon) {
            return ['discount' => $coupon_discount, 'carts' => []];
        }

        $products = [];
        if($coupon->details != null){
            $details = json_decode($coupon->details);
            if($coupon->type == "category_base"){
                foreach($details as $detail){
                    $products = array_merge($products, \App\Models\Product::where('category_id', $detail->category_id)->pluck('id')->toArray());
                }
                // remove from products the excluded products
                foreach($details as $detail){
                    if(isset($detail->excluded_product_ids)){
                        $products = array_diff($products, $detail->excluded_product_ids);
                    }
                }
            }elseif($coupon->type == "brand_base"){
                foreach($details as $detail){
                    $products = array_merge($products, \App\Models\Product::where('brand_id', $detail->brand_id)->pluck('id')->toArray());
                }
                // remove from products the excluded products
                foreach($details as $detail){
                    if(isset($detail->excluded_product_ids)){
        ...
        // if ($current_date < strtotime($coupon->start_date) || $current_date > strtotime($coupon->end_date)) {
            return ['discount' => $coupon_discount, 'carts' => [], 'reason' => 'expired'];
        }

        // Check if the coupon has already been used
        if ($user_id && $useForCalculation == false) {
            $usage = CouponUsage::where('user_id', $user_id)->where('coupon_id', $coupon->id)->first();
        } else {
            // $usage = CouponUsage::where('guest_id', $guest_id)->where('coupon_id', $coupon->id)->first();
            $usage = false;
        }
        ...
        // Retrieve the carts based on user or guest
        if ($user_id) {
            if($coupon->type == "category_base" || $coupon->type == "brand_base"){
        ...
        if ($carts->isEmpty()) {
            return ['discount' => $coupon_discount, 'carts' => $carts];
        }

        $coupon_details = json_decode($coupon->details);
        if (!$coupon_details) {
            Log::error("Coupon details not found or invalid for coupon ID: {$coupon->id}");
            return ['discount' => $coupon_discount, 'carts' => $carts];
        }
        ...
        switch ($coupon->type) {
            case 'cart_base':
                ...
                foreach ($carts as $cartItem) {
                    ...
                    $sum += cart_product_price($cartItem, $product, false, false) * $cartItem->quantity;
                    $sum += cart_product_tax($cartItem, $product, false) * $cartItem->quantity;
                    $sum += $cartItem->shipping_cost;
                }
            ...
            case 'product_base':
                foreach ($carts as $cartItem) {
                    ...
                    foreach ($coupon_details as $detail) {
                        if ($detail->product_id == $cartItem->product_id) {
                            if ($coupon->discount_type == 'percent') {
            ...
            case 'category_base':
                ...
                foreach ($carts as $cartItem) {
                    ...
                    if ($product->category) {
                        foreach ($coupon_details as $detail) {
                            if ($detail->category_id == $product->category_id) {
                                if (isset($detail->excluded_product_ids) && in_array($cartItem->product_id, $detail->excluded_product_ids)) {
                                    continue;
                                }
                                $eligibleCarts[] = $cartItem;
                            }
                        }
                    }
                }

                $total = calculateCartSubTotal($eligibleCarts);

                // if ($total >= ($coupon_details->min_buy ?? 0)) { // Assuming min_buy might not be set for category_base
                //     if ($coupon->discount_type == 'percent') {
                //         $coupon_discount = ($total * $coupon->discount) / 100;
                //         if (isset($coupon_details->max_discount)) {
                //             $coupon_discount = min($coupon_discount, $coupon_details->max_discount);
                //         }
                //     } elseif ($coupon->discount_type == 'amount') {
                //         $coupon_discount = $coupon->discount;
                ...
                foreach ($eligibleCarts as $cart) {
                    if($total >= ($coupon_details->min_buy ?? 0)){
                        if ($coupon->discount_type == 'percent') {
                            $coupon_discount += (cart_product_price($cart, Product::find($cart->product_id), false, false) * $coupon->discount / 100) * $cart->quantity;
                        } elseif ($coupon->discount_type == 'amount') {
                            $coupon_discount += $coupon->discount * $cart->quantity;
                        }
                    }
                }
            ...
            case 'brand_base':
                $brand_ids = array_map(function($detail) {
                    return $detail->brand_id;
                }, $coupon_details);
                ...
                foreach ($carts as $cartItem) {
                    ...
                    if (in_array($product->brand_id, $brand_ids)) {
                        ...
                        foreach ($coupon_details as $detail) {
                            if ($detail->brand_id == $product->brand_id && isset($detail->excluded_product_ids) && in_array($cartItem->product_id, $detail->excluded_product_ids)) {
            ...
            default:
                Log::warning("Unknown coupon type: {$coupon->type} for coupon ID: {$coupon->id}");
                break;
        }

        return ['discount' => $coupon_discount, 'carts' => $carts, 'coupon_id' => $coupon->id];
    }
}

if (!function_exists('carts_coupon_discount')) {
    function carts_coupon_discount($code, $formatted = false, $useForCalculation = false)
    {
        $user = Auth::user();
        ...
        $result = calculate_coupon_discount($code, $user_id, $guest_id, $temp_user_id, $useForCalculation);
        ...
        if($coupon != null) {
            if($coupon->details != null){
                ...
                if($coupon->type == "category_base"){
                    ...
                    foreach($details as $detail){
                        if(isset($detail->excluded_product_ids)){
                            Log::alert("Excluded Product IDs: " . json_encode($detail->excluded_product_ids));
                            $products = array_diff($products, $detail->excluded_product_ids);
                        }
                    }
                }elseif($coupon->type == "brand_base"){
        ...
        if ($coupon_discount > 0 && !$carts->isEmpty()) {
            $discount_per_cart = $coupon_discount / $carts->count();
            if ($user_id) {
                Cart::where('user_id', $user_id)
                ->whereIn('product_id', $products)
                    // ->where('owner_id', $coupon->user_id)
                    ->update(['discount' => $discount_per_cart]);
            } else {
...
if (!function_exists('seller_base_carrier_list')) {
    function seller_base_carrier_list($owner_id, $temp_user_id)
    {
        $carrier_list = array();
        if(auth()->user() != null) {
...
//             $filePath = Storage::path('public/' . $asset->file_name);
...
//             $fileExtension = pathinfo($asset->file_name, PATHINFO_EXTENSION);
//             $allowedExtensions = ['jpg', 'jpeg', 'png', 'gif'];

//             if (in_array(strtolower($fileExtension), $allowedExtensions)) {
//                 $webpFileName = preg_replace('/\.(jpg|jpeg|png|gif)$/i', '.webp', $asset->file_name);

//                 // Convert to WebP if not already converted
//                 if (!Storage::exists('public/' . $webpFileName)) {
//                     try {
//                         $image = Image::make($filePath)->encode('webp', 90);
//                         Storage::put('public/' . $webpFileName, (string) $image);
//                         $asset->file_name = $webpFileName;
//                         $asset->save();
...
if (!function_exists('get_setting')) {
    function get_setting($key,$lang= null, $default = null)
    {
...
function calculateCartSubTotal($carts) {
    $subtotal = 0;
    $tax = 0;
    $shipping = 0;
