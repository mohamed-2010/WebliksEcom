<?php

namespace App\Utility;

use App\Mail\InvoiceEmailManager;
use App\Models\Branche;
use App\Models\BrancheUser;
use App\Models\Guest;
use App\Models\Seller;
use App\Models\Staff;
use App\Models\PickupPoint;
use App\Models\User;
use App\Models\SmsTemplate;
use App\Http\Controllers\OTPVerificationController;
use Mail;
use Illuminate\Support\Facades\Notification;
use App\Notifications\OrderNotification;
use App\Models\FirebaseNotification;
use App\Events\NewOrderNotification;
use App\Services\FirebaseService;

class NotificationUtility
{
    public static function sendOrderPlacedNotification($order, $request = null)
    {       
        //sends email to customer with the invoice pdf attached
        $array['view'] = 'emails.invoice';
        $array['subject'] = translate('A new order has been placed') . ' - ' . $order->code;
        $array['from'] = env('MAIL_FROM_ADDRESS');
        $array['order'] = $order;
        try {
            if($order->user != null) {
                Mail::to($order->user->email)->queue(new InvoiceEmailManager($array));
            }else{
                $guest = Guest::where('id', $order->guest_id)->first();
                Mail::to($guest->email)->queue(new InvoiceEmailManager($array));
            }
            Mail::to($order->orderDetails->first()->product->user->email)->queue(new InvoiceEmailManager($array));
        } catch (\Exception $e) {

        }

        if (addon_is_activated('otp_system') && SmsTemplate::where('identifier', 'order_placement')->first()->status == 1) {
            try {
                $otpController = new OTPVerificationController;
                $otpController->send_order_code($order);
            } catch (\Exception $e) {

            }
        }

        //sends Notifications to user
        self::sendNotification($order, 'placed');
        if ($request !=null && get_setting('google_firebase') == 1 && $order->user->device_token != null) {
            $request->device_token = $order->user->device_token;
            $request->title = "Order placed !";
            $request->text = "An order {$order->code} has been placed";

            $request->type = "order";
            $request->id = $order->id;
            $request->user_id = $order->user->id;

            self::sendFirebaseNotification($request);
        }
    }

    public static function sendNotification($order, $order_status)
    {        
        if ($order->seller_id == \App\Models\User::where('user_type', 'admin')->first()->id) {
            $users = User::findMany([$order->user != null ? $order->user->id : $order->guest_id, $order->seller_id]);
        } else {
            $users = User::findMany([$order->user != null ? $order->user->id : $order->guest_id, $order->seller_id, \App\Models\User::where('user_type', 'admin')->first()->id]);
        }

        $order_notification = array();
        $order_notification['order_id'] = $order->id;
        $order_notification['order_code'] = $order->code;
        $order_notification['user_id'] = $order->user_id;
        $order_notification['seller_id'] = $order->seller_id;
        $order_notification['status'] = $order_status;

        // add the notification to staffs of the seller
        $staffs = Seller::find($order->seller_id);
        if($staffs == null){
            $staffs = Staff::all();
        }else{
            $staffs = $staffs->staff;
        }

        foreach ($staffs as $staff) {
            $branch_user = BrancheUser::with('branche')->with('branche.cities')->where('user_id', $staff->user->id)->get();
            $cities_id = [];
    
            // Extract the IDs of the cities assigned to the staff
            foreach ($branch_user as $key => $value) {
                if($value->branche != null) {
                    $cities_id[] = $value->branche->cities->pluck('id')->toArray();
                }
            }

            //return $cities_id;

            //$orders = $orders->where('shipping_address', 'like', '%' . $cities_id[0][0] . '%');
            foreach ($cities_id as $key => $value) {
                foreach ($value as $key => $value2) {
                    if (strpos($order->shipping_address, $value2) !== false) {
                        $users[] = $staff->user;
                    }
                }
            }
        }
        
        // check if order is pickup_point send for manager of it
        if($order->shipping_type == 'pickup_point') {
            $pickup_point = PickupPoint::where('id', $order->pickup_point_id)->first();
            if($pickup_point != null) {
                $staf = Staff::where('id', $pickup_point->staff_id)->first();
                if($staf != null) {
                    $users[] = $staf->user;
                }
            }
        }
        
        $array['view'] = 'emails.invoice';
        $array['subject'] = translate('A new order has been placed') . ' - ' . $order->code;
        $array['from'] = env('MAIL_FROM_ADDRESS');
        $array['order'] = $order;
        
        event(new NewOrderNotification($order_notification));
        Notification::send($users, new OrderNotification($order_notification));

        // Send FCM notification
        self::sendFcmNotification($order, $order_status);
        
        
        try {
            foreach($users as $user) {
                $send = Mail::to($user->email)->queue(new InvoiceEmailManager($array));
            }
        } catch (\Exception $e) {
            \Log::alert($e);
        }

    }

    public static function sendFcmNotification($order, $order_status)
    {
        // Define topics based on the order and user roles
        $topics = [];
        
        // get staf of the seller
        $staffs = Seller::find($order->seller_id);
        if($staffs == null){
            $staffs = Staff::all();
        }else{
            $staffs = $staffs->staff;
        }
        foreach ($staffs as $staff) {
            $branch_user = BrancheUser::with('branche')->with('branche.cities')->where('user_id', $staff->user->id)->get();
            $cities_id = [];
    
            // Extract the IDs of the cities assigned to the staff
            foreach ($branch_user as $key => $value) {
                if($value->branche != null) {
                    $cities_id[] = $value->branche->cities->pluck('id')->toArray();
                }
            }

            //return $cities_id;

            //$orders = $orders->where('shipping_address', 'like', '%' . $cities_id[0][0] . '%');
            foreach ($cities_id as $key => $value) {
                foreach ($value as $key => $value2) {
                    if (strpos($order->shipping_address, $value2) !== false) {
                        $topics[] = $staff->user->email;
                    }
                }
            }
            // $topics[] = $staff->user->email;
        }
        
        // check if order is pickup_point send for manager of it
        if($order->shipping_type == 'pickup_point'){
            $pickup_point = PickupPoint::where('id', $order->pickup_point_id)->first();
            if($pickup_point != null) {
                $staf = Staff::where('id', $pickup_point->staff_id)->first();
                if($staf != null) {
                    $topics[] = $staf->user->email;
                }
            }
        }

        // Set notification content
        $title = get_setting('website_name') . ' store has a new order';
        $body = "Order has been placed #{$order->id}";

        // Send notification to multiple topics
        $firebaseService = new FirebaseService();
        $firebaseService->sendNotificationToTopics($topics, $title, $body);
    }

    public static function sendFirebaseNotification($req)
    {        
        $url = 'https://fcm.googleapis.com/v1/projects/webliks-dashboard/messages:send';

        $fields = array
        (
            'to' => $req->device_token,
            'notification' => [
                'body' => $req->text,
                'title' => $req->title,
                'sound' => 'default' /*Default sound*/
            ],
            "android" => [
                "priority" => "high",
            ],
            "apns" => [
                "payload" => [
                    "aps" => [
                        "sound" => "default",
                    ],
                ],
            ],
            "webpush" => [
                "headers" => [
                    "Urgency" => "high",
                ],
            ],
            'data' => [
                'item_type' => $req->type,
                'item_type_id' => $req->id,
                'click_action' => 'FLUTTER_NOTIFICATION_CLICK'
            ]
        );

        //$fields = json_encode($arrayToSend);
        $headers = array(
            'Authorization: key=' . env('FCM_SERVER_KEY'),
            'Content-Type: application/json'
        );

        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        curl_setopt($ch, CURLOPT_POST, true);
        curl_setopt($ch, CURLOPT_HTTPHEADER, $headers);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($fields));

        $result = curl_exec($ch);
        curl_close($ch);

        $firebase_notification = new FirebaseNotification;
        $firebase_notification->title = $req->title;
        $firebase_notification->text = $req->text;
        $firebase_notification->item_type = $req->type;
        $firebase_notification->item_type_id = $req->id;
        $firebase_notification->receiver_id = $req->user_id;

        $firebase_notification->save();
    }
}
